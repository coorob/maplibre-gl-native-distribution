...
| Tag | Base | Fix | Slices |
|-----|------|-----|--------|
| `6.27.0-traska.97` | MapLibre Native [`ios-v6.27.0`](https://github.com/maplibre/maplibre-native/releases/tag/ios-v6.27.0) + accepted Traska renderer [`2897bfba`](https://github.com/coorob/maplibre-native/commit/2897bfba5c745d459d6958a55581698be668d938) | Stabilizes terrain-draped hillshade and land color, clips the 3D sea plane against coastal terrain without flooding or jagged shorelines, and promotes the physically accepted iPhone 13 memory controls to rollback-safe production defaults | `ios-arm64` (device) + `ios-arm64_x86_64-simulator` |
...
`6.27.0-traska.97`. The repo is public, so SwiftPM downloads the framework anonymously — no auth.
...
```ruby
$MLRN_NATIVE_VERSION = "6.27.0-traska.97"
$MLRN_SPM_SPEC = {
  url: "https://github.com/coorob/maplibre-gl-native-distribution",
  requirement: { kind: "exactVersion", version: "6.27.0-traska.97" },
...
ASSET=/tmp/MapLibre.xcframework.zip
cp "$ZIP" "$ASSET"                                          # GitHub uses this exact basename
swift package compute-checksum "$ASSET"                     # new checksum
gh release create 6.27.0-traska.97 "$ASSET" --repo coorob/maplibre-gl-native-distribution
